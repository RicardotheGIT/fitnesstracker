...
export default function WorkoutPage() {
  ...
  useEffect(() => {
    ...
    // Tune: rising C-major arpeggio fanfare using the existing beep()
    beep(523, 0.2, 0);
    beep(659, 0.2, 0.22);
    beep(784, 0.2, 0.44);
    beep(1047, 0.45, 0.66);
    ...
    // Confetti: DOM elements appended straight to body so they're always on top
    const colors = ['#ff6b6b', '#ffd93d', '#6bcb77', '#4d96ff', '#f7a8d8', '#a78bfa', '#ff9f43'];
    const pieces: HTMLDivElement[] = [];
    const vels: { vx: number; vy: number; g: number }[] = [];
    ...
    for (let i = 0; i < 80; i++) {
      ...
      el.style.cssText = `position:fixed;left:${x}px;top:${y}px;width:${size}px;height:${size}px;background:${colors[i % colors.length]};border-radius:${Math.random() > 0.5 ? '50%' : '2px'};pointer-events:none;z-index:9999;opacity:1;`;
      document.body.appendChild(el);
      pieces.push(el);
      vels.push({ vx: (Math.random() - 0.5) * 6, vy: 2 + Math.random() * 5, g: 0.15 + Math.random() * 0.15 });
    }
    ...
    const animate = () => {
      ...
      pieces.forEach((el, i) => {
        const v = vels[i];
        const top = parseFloat(el.style.top) + v.vy;
        const left = parseFloat(el.style.left) + v.vx;
        v.vy += v.g;
        el.style.top = top + 'px';
        el.style.left = left + 'px';
        if (top > window.innerHeight + 20) { el.style.opacity = '0'; } else { alive = true; }
      });
    ...
    return () => {
      ...
      pieces.forEach(el => { if (el.parentNode) el.parentNode.removeChild(el); });
      confettiRef.current = [];
    };
  }, [phase, beep]);
  ...
  const start = () => {
    ...
    confettiRef.current.forEach(el => { if (el.parentNode) el.parentNode.removeChild(el); });
    confettiRef.current = [];
