...
export default function App({ user }: { user: User }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      setHistory(mergeHistory(workouts, diets));
      setLoaded(true);
    })();
  }, [user.id]);

  const saveEntry = useCallback(async (entry: Partial<HistoryEntry>) => {
    if (!entry.date) return;

    if (entry.workout) {
      const { workout: w } = entry;
      await supabase.from('workout_sessions').upsert(
        { user_id: user.id, date: entry.date, weight_kg: w.weightKg, cal_during: w.calDuring, cal_total: w.calTotal, done: w.done },
    ...
    if (entry.diet) {
      ...
      await supabase.from('diet_logs').upsert(
        { user_id: user.id, date: entry.date, status: d.status, surplus_cal: d.surplus },
        { onConflict: 'user_id,date' }
      );
    }
    ...
    setHistory((prev) => {
      ...
      return [entry as HistoryEntry, ...prev].slice(0, 120);
    });
  }, [user.id]);
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0f', display: 'flex', flexDirection: 'column', alignItems: 'center', fontFamily: "'DM Mono','Courier New',monospace", color: '#e0e0e0', padding: '24px 16px 48px' }}>
      ...
      {page === 'workout' && <WorkoutPage saveEntry={saveEntry} />}
